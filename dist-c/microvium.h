...
typedef enum mvm_TeError {
  ...
  /* 45 */ MVM_E_JS_EXCEPTION, // The JS code threw an exception, and there is a catch, but there is a native (host) frame in the stack before the catch
  /* 46 */ MVM_E_FATAL_ERROR_MUST_KILL_VM, // Please make sure that MVM_FATAL_ERROR does not return, or bad things can happen. (Kill the process, the thread, or use longjmp)
} mvm_TeError;
